...
REM 설치할 프로그램 이름 설정
set PROGRAM_NAME="AWS Command Line Interface"
set DOWNLOAD_URL=https://awscli.amazonaws.com/AWSCLIV2.msi
set DOWNLOAD_FILE=%TEMP%\AWSCLIV2.msi

REM AWS CLI가 이미 설치되어 있는지 확인
where aws >nul 2>&1
if %ERRORLEVEL% equ 0 (
    echo %PROGRAM_NAME% is already installed. Exiting...
    aws --version
    goto AWS_install_end
)
...
msiexec /i "%DOWNLOAD_FILE%" /qn /norestart
if %ERRORLEVEL% neq 0 (
